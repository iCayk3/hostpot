...
export default function Home() {
  ...
  async function reopenPayment(payment:{id:string;mac:string}){const answer=await openDialog({title:"Liberar mais 2 minutos",message:`Deseja reabrir a janela de pagamento por mais 2 minutos para ${payment.mac}? O plano ainda não será liberado.`,confirmLabel:"Liberar 2 minutos"});if(!answer.confirmed)return;const response=await fetch(`/api/payments/reopen/${payment.id}`,{method:"POST"});const payload=await response.json().catch(()=>({})) as {error?:string};if(!response.ok)return notify(payload.error||"Não foi possível reabrir a janela.");notify("Mais 2 minutos para pagamento enviados ao MikroTik.")}
          ...
          <div className="admin-content">
            ...
            <article className="sessions-card">
              ...
              <div className="session-table">
                ...
                {paymentDiagnostics.length===0?<div className="empty">Nenhuma tentativa de pagamento registrada.</div>:paymentDiagnostics.map(payment=><div className="table-row" key={payment.id}><span><strong>{payment.identity}</strong><small>{payment.mac}</small></span><span>{payment.minutes} min · {Number(payment.amount).toLocaleString("pt-BR",{style:"currency",currency:"BRL"})}</span><span><b className={payment.status==="approved"||payment.status==="paid_cash"?"status-active":""}>{payment.status}</b></span><span title={payment.last_error||payment.mp_payment_id||""}>{payment.last_error||payment.mp_payment_id||"Aguardando resposta"}</span><span>{payment.released?<b className="status-active">Liberado</b>:<span className="session-actions"><button className="text-button" onClick={()=>reopenPayment(payment)}>Mais 2 min</button><button className="text-button" onClick={()=>receiveCash(payment)}>Recebi em espécie</button></span>}</span></div>)}
              </div>
            </article>
