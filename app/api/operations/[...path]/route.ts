...
import { acknowledgeCommand,adminOverview,agentTokenHash,allOperationalDevices,authenticateOperator,createOperator,deleteOperator,deviceDashboard,getOperator,listOperators,nextCommand,operatorDevices,queueRelease,queueTerminate,saveTelemetry,setClientLabel,updateOperatorDevices } from "@/lib/operations-store";
...
export async function POST(request:Request,ctx:Ctx){try{const{path}=await ctx.params;
 ...
 if(path[0]==="command-ack"&&path[1]&&path[2]){enforceRateLimit(request,"router-command-ack",30,60_000,path[1]);return acknowledgeCommand(agentTokenHash(path[1]),path[2])?json({status:"confirmed"}):json({error:"Comando ou token inválido"},401)}
