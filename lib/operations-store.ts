...
export function queueRelease(deviceId:string,mac:string,minutes:number,operatorId:string){
  ...
  const pixWindows=!paymentWindow?db.prepare("SELECT id FROM access_releases WHERE device_id=? AND mac=? AND created_by='pix-window' AND status IN ('queued','delivered')").all(deviceId,safeMac) as Array<{id:string}>:[];
  const pixCleanup=pixWindows.map(window=>{const pixTag=`conecta-pix-${window.id.slice(0,8)}`;return `/ip hotspot walled-garden remove [find comment=${pixTag}]\n/queue simple remove [find name=${pixTag}]\n/system scheduler remove [find name=${pixTag}]`}).join("\n");
  if(pixWindows.length){const placeholders=pixWindows.map(()=>"?").join(","),ids=pixWindows.map(item=>item.id);db.prepare(`UPDATE access_releases SET status='superseded' WHERE id IN (${placeholders})`).run(...ids);db.prepare(`UPDATE router_commands SET status='cancelled' WHERE id IN (${placeholders}) AND status IN ('pending','sent')`).run(...ids)}
  const id=randomUUID(), tag=`conecta-${id.slice(0,8)}`, script=`:local clientAddress ""\n:local clientHost [/ip hotspot host find where mac-address=${safeMac}]\n:if ([:len \$clientHost] > 0) do={:set clientAddress [/ip hotspot host get [:pick \$clientHost 0] address]}\n${pixCleanup}\n/ip hotspot ip-binding remove [find mac-address=${safeMac}]\n/ip hotspot ip-binding add mac-address=${safeMac} type=bypassed comment=\"${tag}\"\n/queue simple remove [find name=${tag}]\n:if ([:len \$clientAddress] > 0) do={/queue simple add name=${tag} target=(\$clientAddress . "/32") max-limit=${rate} comment=\"Conecta+ limite temporario\"}\n/system scheduler remove [find name=${tag}]\n/system scheduler add name=${tag} interval=${minutes}m on-event=\"/ip hotspot ip-binding remove [find comment=${tag}]; /queue simple remove [find name=${tag}]; /system scheduler set [find name=${tag}] disabled=yes\"\n:log info \"Conecta+: acesso ${minutes}m iniciado apos encerramento da janela de pagamento para ${safeMac}\"`;
...
export function nextCommand(tokenHash:string,token?:string,base?:string){
  ...
  return expired.created_by==="pix-window"?`/ip hotspot walled-garden remove [find comment=${tag}]\n/queue simple remove [find name=${tag}]\n/system scheduler remove [find name=${tag}]\n:log info \"Conecta+: janela Pix expirada e removida pelo servidor\"`:`/ip hotspot active remove [find where user~\"${tag}\"]\n/ip hotspot ip-binding remove [find comment=${tag}]\n/queue simple remove [find name=${tag}]\n/system scheduler remove [find name=${tag}]\n:log info \"Conecta+: acesso ${tag} expirado sem interferir em liberacoes posteriores\"`;
}
