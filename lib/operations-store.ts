...
export function updateOperatorDevices(operatorId:string,deviceIds:string[]){if(!db.prepare("SELECT 1 FROM operators WHERE id=?").get(operatorId))return false;const valid=[...new Set(deviceIds.slice(0,100))].filter(id=>db.prepare("SELECT 1 FROM devices WHERE id=?").get(id));db.exec("BEGIN");try{db.prepare("DELETE FROM operator_devices WHERE operator_id=?").run(operatorId);const insert=db.prepare("INSERT INTO operator_devices VALUES (?,?)");for(const id of valid)insert.run(operatorId,id);db.exec("COMMIT");return true}catch(error){db.exec("ROLLBACK");throw error}}
export function deleteOperator(operatorId:string){db.exec("BEGIN");try{db.prepare("DELETE FROM operator_devices WHERE operator_id=?").run(operatorId);const result=db.prepare("DELETE FROM operators WHERE id=?").run(operatorId);db.exec("COMMIT");return result.changes>0}catch(error){db.exec("ROLLBACK");throw error}}
export function adminOverview(){const devices=db.prepare("SELECT id FROM devices ORDER BY last_seen DESC").all() as Array<{id:string}>;const dashboards=devices.map(row=>deviceDashboard(row.id)).filter(Boolean) as any[];const sessions=dashboards.flatMap(item=>item.sessions.map((session:any)=>({...session,identity:item.device.identity}))).sort((a:any,b:any)=>String(b.updated_at||"").localeCompare(String(a.updated_at||""))).slice(0,50);const updated=dashboards.map(item=>item.device.updated_at).filter(Boolean).sort().at(-1)||null;return{totals:{routers:devices.length,active:dashboards.reduce((sum,item)=>sum+Number(item.device.active_count||0),0),hosts:dashboards.reduce((sum,item)=>sum+Number(item.device.host_count||0),0),updated_at:updated},sessions}}
...
export function deviceDashboard(deviceId: string, operatorId?: string) {
  ...
  const sessions = db.prepare(`SELECT s.username,s.address,s.mac,s.uptime,s.time_left,s.updated_at,c.label,c.detected_name FROM hotspot_sessions s LEFT JOIN client_devices c ON c.device_id=s.device_id AND c.mac=s.mac WHERE s.device_id=? ORDER BY s.updated_at DESC`).all(deviceId) as any[];
  ...
  const nativeMacs=new Set(sessions.map(row=>String(row.mac||"").toUpperCase()));
  for(const access of adminAccesses){const mac=String(access.mac||"").toUpperCase();if(access.status!=="delivered"||!mac||nativeMacs.has(mac)||Number(access.remaining_seconds)<=0)continue;const host=(hosts as any[]).find(row=>String(row.mac||"").toUpperCase()===mac);if(!host)continue;const remaining=Number(access.remaining_seconds),elapsed=Math.max(0,Number(access.minutes)*60-remaining);sessions.push({username:`portal-${access.minutes}m`,address:host.address||"",mac,uptime:`${Math.floor(elapsed/60)}m ${elapsed%60}s`,time_left:`${String(Math.floor(remaining/60)).padStart(2,"0")}:${String(remaining%60).padStart(2,"0")}`,updated_at:host.updated_at,label:access.label||host.label,detected_name:access.detected_name||host.detected_name})}
  ...
  for(const row of hosts as any[]){const mac=String(row.mac||"").toUpperCase();if(mac&&(row.authorized||releasedMacs.has(mac)))activeMacs.add(mac)}
  device.active_count=activeMacs.size;
...
export function saveTelemetry(tokenHash: string, values: Record<string,string>) {
  ...
  const connected=db.prepare(`SELECT COUNT(DISTINCT mac) total FROM (SELECT mac FROM hotspot_sessions WHERE device_id=? AND mac<>'' UNION ALL SELECT h.mac FROM hotspot_hosts h WHERE h.device_id=? AND h.mac<>'' AND (h.authorized=1 OR EXISTS (SELECT 1 FROM access_releases r WHERE r.device_id=h.device_id AND r.mac=h.mac AND r.status IN ('queued','delivered') AND (r.executed_at IS NULL OR datetime(r.executed_at,'+'||r.minutes||' minutes')>datetime('now')))))`).get(device.id,device.id) as {total:number};
  db.prepare("UPDATE telemetry SET active_count=? WHERE device_id=?").run(Number(connected.total||0),device.id);
  return true;
}
...
export function queueRelease(deviceId:string,mac:string,minutes:number,operatorId:string){
  ...
  const previous=!paymentWindow?db.prepare("SELECT id,created_by FROM access_releases WHERE device_id=? AND mac=? AND status IN ('queued','delivered')").all(deviceId,safeMac) as Array<{id:string;created_by:string}>:[];
  const previousCleanup=previous.map(item=>{const previousTag=item.created_by==="pix-window"?`conecta-pix-${item.id.slice(0,8)}`:`conecta-${item.id.slice(0,8)}`;return item.created_by==="pix-window"?`/ip hotspot walled-garden remove [find comment=${previousTag}]\n/queue simple remove [find name=${previousTag}]\n/system scheduler remove [find name=${previousTag}]`:`/ip hotspot ip-binding remove [find comment=${previousTag}]\n/queue simple remove [find name=${previousTag}]\n/system scheduler remove [find name=${previousTag}]`}).join("\n");
  if(previous.length){const placeholders=previous.map(()=>"?").join(","),ids=previous.map(item=>item.id);db.prepare(`UPDATE access_releases SET status='superseded' WHERE id IN (${placeholders})`).run(...ids);db.prepare(`UPDATE router_commands SET status='cancelled' WHERE id IN (${placeholders}) AND status IN ('pending','sent')`).run(...ids)}
  const id=randomUUID(), tag=`conecta-${id.slice(0,8)}`, script=`:local clientAddress ""\n:local clientHost [/ip hotspot host find where mac-address=${safeMac}]\n:if ([:len \$clientHost] > 0) do={:set clientAddress [/ip hotspot host get [:pick \$clientHost 0] address]}\n${previousCleanup}\n/ip hotspot ip-binding remove [find mac-address=${safeMac}]\n/ip hotspot ip-binding add mac-address=${safeMac} type=bypassed comment=\"${tag}\"\n/queue simple remove [find name=${tag}]\n:if ([:len \$clientAddress] > 0) do={/queue simple add name=${tag} target=(\$clientAddress . "/32") max-limit=${rate} comment=\"Conecta+ limite temporario\"}\n/system scheduler remove [find name=${tag}]\n/system scheduler add name=${tag} interval=${minutes}m on-event=\":local accessBinding [/ip hotspot ip-binding find comment=${tag}]; :if ([:len \\$accessBinding] > 0) do={/ip hotspot ip-binding remove \\$accessBinding; /ip hotspot active remove [find mac-address=${safeMac}]; /ip hotspot host remove [find mac-address=${safeMac}]}; /queue simple remove [find name=${tag}]; /system scheduler set [find name=${tag}] disabled=yes\"\n:log info \"Conecta+: acesso ${minutes}m iniciado apos encerramento da janela de pagamento para ${safeMac}\"`;
...
export function queuePaymentWindow(deviceId:string,address:string,mac:string){
  const safeMac=validMac(mac);if(!safeMac)return null;
...
export function acknowledgeCommand(tokenHash:string,commandId:string){const device=deviceIdByTokenHash(tokenHash);if(!device)return false;const command=db.prepare("SELECT id FROM router_commands WHERE id=? AND device_id=? AND status='sent'").get(commandId,device.id);if(!command)return false;const timestamp=now();db.prepare("UPDATE router_commands SET status='delivered',delivered_at=? WHERE id=?").run(timestamp,commandId);db.prepare("UPDATE access_releases SET status='delivered',executed_at=? WHERE id=? AND status='queued'").run(timestamp,commandId);const paymentTable=db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='pix_payments'").get();if(paymentTable){const columns=db.prepare("PRAGMA table_info(pix_payments)").all() as Array<{name:string}>;if(columns.some(column=>column.name==="release_id"))db.prepare("UPDATE pix_payments SET released=1,updated_at=? WHERE release_id=?").run(timestamp,commandId)}return true}

export function nextCommand(tokenHash:string,token?:string,base?:string){
  const device=deviceIdByTokenHash(tokenHash);if(!device)return null;
  const command=db.prepare("SELECT * FROM router_commands WHERE device_id=? AND (status='pending' OR (status='sent' AND datetime(delivered_at)<=datetime('now','-30 seconds'))) ORDER BY created_at LIMIT 1").get(device.id) as any;
  if(command){db.prepare("UPDATE router_commands SET status='sent',delivered_at=? WHERE id=?").run(now(),command.id);let host="";try{host=new URL(base||"").hostname}catch{}const cleanBase=(base||"").replace(/\/$/,""),script=String(command.script).replaceAll("__CONNECTION_TOKEN__",token||"").replaceAll("__CONNECTION_BASE__",cleanBase).replaceAll("__CONNECTION_HOST__",host),ack=`${cleanBase}/api/operations/command-ack/${token||""}/${command.id}`;return `/ip firewall filter disable [find action=fasttrack-connection]\n${script}\n/tool fetch url=\"${ack}\" http-method=post http-data=\"status=ok\" keep-result=no\n:log info \"Conecta+: comando ${command.id} confirmado\"`}
  const releases=db.prepare("SELECT id,mac,minutes,created_by,executed_at FROM access_releases WHERE device_id=? AND status='delivered' AND executed_at IS NOT NULL ORDER BY executed_at").all(device.id) as Array<{id:string;mac:string;minutes:number;created_by:string;executed_at:string}>;
  const expired=releases.find(item=>new Date(item.executed_at).getTime()+Number(item.minutes)*60_000<=Date.now());
  if(!expired)return ":nothing";
  const tag=expired.created_by==="pix-window"?`conecta-pix-${expired.id.slice(0,8)}`:`conecta-${expired.id.slice(0,8)}`;db.prepare("UPDATE access_releases SET status='expired' WHERE id=?").run(expired.id);
  return expired.created_by==="pix-window"?`/ip hotspot walled-garden remove [find comment=${tag}]\n/queue simple remove [find name=${tag}]\n/system scheduler remove [find name=${tag}]\n:log info \"Conecta+: janela Pix expirada e removida pelo servidor\"`:`:local accessBinding [/ip hotspot ip-binding find comment=${tag}]\n:if ([:len \$accessBinding] > 0) do={/ip hotspot ip-binding remove \$accessBinding; /ip hotspot active remove [find mac-address=${expired.mac}]; /ip hotspot host remove [find mac-address=${expired.mac}]}\n/queue simple remove [find name=${tag}]\n/system scheduler remove [find name=${tag}]\n:log info \"Conecta+: acesso ${tag} expirado e dispositivo devolvido ao portal\"`;
}
