...
export function queueRelease(deviceId:string,mac:string,minutes:number,operatorId:string){
  ...
  if(pixWindows.length){const placeholders=pixWindows.map(()=>"?").join(","),ids=pixWindows.map(item=>item.id);db.prepare(`UPDATE access_releases SET status='superseded' WHERE id IN (${placeholders})`).run(...ids);db.prepare(`UPDATE router_commands SET status='cancelled' WHERE id IN (${placeholders}) AND status='pending'`).run(...ids)}
...
export function nextCommand(tokenHash:string,token?:string,base?:string){const d=deviceIdByTokenHash(tokenHash);if(!d)return null;const releases=db.prepare("SELECT id,mac,minutes,created_by,executed_at FROM access_releases WHERE device_id=? AND status='delivered' AND executed_at IS NOT NULL ORDER BY executed_at").all(d.id) as Array<{id:string;mac:string;minutes:number;created_by:string;executed_at:string}>;const expired=releases.find(item=>new Date(item.executed_at).getTime()+Number(item.minutes)*60_000<=Date.now());if(expired){const tag=expired.created_by==="pix-window"?`conecta-pix-${expired.id.slice(0,8)}`:`conecta-${expired.id.slice(0,8)}`;db.prepare("UPDATE access_releases SET status='expired' WHERE id=?").run(expired.id);return expired.created_by==="pix-window"?`/ip hotspot walled-garden remove [find comment=${tag}]\n/queue simple remove [find name=${tag}]\n/system scheduler remove [find name=${tag}]\n:log info \"Conecta+: janela Pix expirada e removida pelo servidor\"`:`/ip hotspot active remove [find mac-address=${expired.mac}]\n/ip hotspot ip-binding remove [find mac-address=${expired.mac}]\n/queue simple remove [find name=${tag}]\n/system scheduler remove [find name=${tag}]\n:log info \"Conecta+: acesso expirado e removido pelo servidor para ${expired.mac}\"`;}const c=db.prepare("SELECT * FROM router_commands WHERE device_id=? AND status='pending' ORDER BY created_at LIMIT 1").get(d.id) as any;if(!c)return ":nothing";db.prepare("UPDATE router_commands SET status='delivered',delivered_at=? WHERE id=?").run(now(),c.id);db.prepare("UPDATE access_releases SET status='delivered',executed_at=? WHERE id=?").run(now(),c.id);let host="";try{host=new URL(base||"").hostname}catch{}const script=String(c.script).replaceAll("__CONNECTION_TOKEN__",token||"").replaceAll("__CONNECTION_BASE__",(base||"").replace(/\/$/,"")).replaceAll("__CONNECTION_HOST__",host);return `/ip firewall filter disable [find action=fasttrack-connection]\n${script}`}
